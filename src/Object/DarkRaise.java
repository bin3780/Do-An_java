...
public class DarkRaise extends PartiObject {
    ...
    public DarkRaise(float x, float y, WorldState gameWorld) {
        super(x, y, 127, 89, 0, 100, gameWorld);
        backAnim = Loader_CacheData.getInstance().getAnimation("darkraise");
        forwardAnim = Loader_CacheData.getInstance().getAnimation("darkraise");
    ...
    public void attack() {
        ...
        float ShinobiX = getGameWorld().shinobi.getPosX();
        float ShinobiY = getGameWorld().shinobi.getPosY();

        float deltaX = ShinobiX - getPosX();
        float deltaY = ShinobiY - getPosY();
